...
abstract class MusicalEntity implements MusicalEntityInterface
{
    ...
    abstract public function getSafeTitle(): string;
    ...
    */
    public static function parse(string $str): array
    {
        ...
        $extra_info['is_cover'] = (
            strlen($str) > 8
            && (
                (Track::TYPE == static::TYPE && 1 === preg_match('/[\-\—\–\(\[].*(originally\sperformed|cover|tribute|karaoke)/iu', $str))
                || (Album::TYPE == static::TYPE && 1 === preg_match('/.*(originally\sperformed|cover(.*)(vol|sessions)|tribute|karaoke)/iu', $str))
            )
        );
