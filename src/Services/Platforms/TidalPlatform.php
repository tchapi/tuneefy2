...
class TidalPlatform extends Platform implements WebStreamingPlatformInterface
{
    ...
    public function expandPermalink(string $permalink, int $mode): PlatformResult
    {
        ...
        if (preg_match(self::REGEX_TIDAL_TRACK, $permalink, $match)) {
            ...
            $musical_entity = new Track(new Album($entity->album->title, $entity->artist->name, $this->getCoverUrlFromCoverHash($entity->album->cover)), $entity->title);
            ...
            $entity = $response->data;
            $musical_entity = new Album($entity->title, $entity->artist->name, $this->getCoverUrlFromCoverHash($entity->cover));
            $musical_entity->addLink(static::TAG, $entity->url);
    ...
    public function extractSearchResults(\stdClass $response, int $type, string $query, int $limit, int $mode): array
    {
        ...
        for ($i = 0; $i < $length; ++$i) {
            ...
            if (Platform::SEARCH_TRACK === $type) {
                $musical_entity = new Track(new Album($current_item->album->title, $current_item->artist->name, $this->getCoverUrlFromCoverHash($current_item->album->cover)), $current_item->title);
                $musical_entity->addLink(static::TAG, str_replace('http://', 'https://', $current_item->url));
            } else { /* if ($type === Platform::SEARCH_ALBUM) */
                $musical_entity = new Album($current_item->title, $current_item->artist->name, $this->getCoverUrlFromCoverHash($current_item->cover));
                $musical_entity->addLink(static::TAG, str_replace('http://', 'https://', $current_item->url));
            }
