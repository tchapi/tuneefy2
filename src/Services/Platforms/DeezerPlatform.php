...
class DeezerPlatform extends Platform implements WebStreamingPlatformInterface
{
    ...
    public function expandPermalink(string $permalink, int $mode): PlatformResult
    {
        ...
        if (preg_match(self::REGEX_DEEZER_TRACK, $permalink, $match)) {
            ...
            $musical_entity = new Track(new Album($entity->album->title, $entity->artist->name, $entity->album->cover), $entity->title);
            ...
            $entity = $response->data;
            $musical_entity = new Album($entity->title, $entity->artist->name, $entity->cover);
            $musical_entity->addLink(static::TAG, $permalink);
    ...
    public function extractSearchResults(\stdClass $response, int $type, string $query, int $limit, int $mode): array
    {
        ...
        for ($i = 0; $i < $length; ++$i) {
            ...
            if (Platform::SEARCH_TRACK === $type) {
                ...
                $musical_entity = new Track(new Album($current_item->album->title, $current_item->artist->name, $picture), $current_item->title);
                ...
                $musical_entity = new Album($current_item->title, $current_item->artist->name, $picture);
                $musical_entity->addLink(static::TAG, $current_item->link);
            }
