    ...
    public function expandPermalink(string $permalink, int $mode): PlatformResult
    {
        ...
        if (preg_match(self::REGEX_SPOTIFY_ALL, $permalink, $match)) {
            ...
            if (Platform::LOOKUP_TRACK === $object_type) {
                $musical_entity = new Track(new Album($entity->album->name, $entity->artists[0]->name, $entity->album->images[1]->url), $entity->name);
                ...
                $musical_entity = new Album($entity->name, $entity->artists[0]->name, $entity->images[1]->url);
                $musical_entity->addLink(static::TAG, $entity->external_urls->spotify);
    ...
    public function extractSearchResults(\stdClass $response, int $type, string $query, int $limit, int $mode): array
    {
        ...
        for ($i = 0; $i < $length; ++$i) {
            ...
            if (Platform::SEARCH_TRACK === $type) {
                ...
                $musical_entity = new Track(new Album($current_item->album->name, $current_item->artists[0]->name, isset($images[1]) ? $images[1]->url : (isset($images[0]) ? $images[0]->url : null)), $current_item->name);
                ...
                $musical_entity = new Album($current_item->name, $current_item->artists[0]->name, isset($images[1]) ? $images[1]->url : (isset($images[0]) ? $images[0]->url : null));
                $musical_entity->addLink(static::TAG, $current_item->external_urls->spotify);
                $externalIds = [static::TAG => $current_item->id];
