...
class OAuthUtil
{
    public static function urlencode_rfc3986($input)//: mixed
    {
        if (is_array($input)) {
            return array_map(function ($e) { return OAuthUtil::urlencode_rfc3986($e); }, $input);
  ...
  public static function urldecode_rfc3986(string $string): array
  {
  ...
  public static function split_header(string $header, bool $only_allow_oauth_parameters = true): array
  {
      ...
      if (preg_match_all('/('.($only_allow_oauth_parameters ? 'oauth_' : '').'[a-z_-]*)=(:?"([^"]*)"|([^,]*))/', $header, $matches)) {
          foreach ($matches[1] as $i => $h) {
              if ($h !== 'realm') {
                  $params[$h] = self::urldecode_rfc3986(array_key_exists($i, $matches[3]) ? $matches[4][$i] : $matches[3][$i]);
              }
          }
      }
  ...
  public static function parse_parameters(string $input = null): array
  {
      if ($input === null) {
          return [];
      }
      ...
      foreach ($pairs as $pair) {
          ...
          $value = array_key_exists(1, $split) ? self::urldecode_rfc3986($split[1]) : '';
